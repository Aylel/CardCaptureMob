...
public class ZombieAttackGoal extends Goal {
    ...
    public ZombieAttackGoal(ZombieFamiliarEntity familiar, double radius, float speed) {
    ...
    private LivingEntity findTarget() {
        if (familiar.getBehaviorMode() == BehaviorMode.AGGRESSIVE) {
            double radius = ZombieFamiliarEntity.AGGRESSIVE_ATTACK_RADIUS;
            AABB range = new AABB(
                    familiar.getX() - radius, familiar.getY() - 4.0, familiar.getZ() - radius,
                    familiar.getX() + radius, familiar.getY() + 4.0, familiar.getZ() + radius
            );
            // --- Modification ici : trie par distance croissante ---
