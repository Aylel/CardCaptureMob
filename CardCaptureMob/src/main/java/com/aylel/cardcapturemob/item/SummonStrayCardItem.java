...
import com.aylel.cardcapturemob.entity.CardFamiliarOwner;
...
public abstract class SummonStrayCardItem<T extends CardFamiliarOwner> extends Item {

    public SummonStrayCardItem(Properties properties) {
        super(properties);
    }

    protected abstract EntityType<? extends StrayFamiliarEntity> getEntityType();
    ...
    public InteractionResultHolder<ItemStack> use(Level world, Player player, InteractionHand hand) {
        ...
        if (!world.isClientSide) {
            ...
            List<Entity> existing = world.getEntities(
                    ...
                    e -> e instanceof CardFamiliarOwner owner && owner.getOwnerUUID().equals(ownerId)
            );
            if (!existing.isEmpty()) {
                for (Entity e : existing) {
                    if (e instanceof StrayFamiliarEntity stray) {
                        UUID entityUuid = stray.getFamiliarUuid();
                        if (entityUuid != null) {
                            FamiliarData entityData = FamiliarManager.get().getFamiliarData(entityUuid);
                            if (entityData != null) {
                                entityData.setCurrentHp(stray.getHealth());
            ...
            // ====== Si carte KO, tente réanimation avec parchemin ======
            ...
            EntityType<? extends StrayFamiliarEntity> type = getEntityType();
            StrayFamiliarEntity ent = type.create(srv);
